...
#ifdef _WIN32
#define _WIN32_WINNT 0x0400 //wtf! need this for wincrypt.h to be even looked at
#include <windows.h>
#include <wincrypt.h>
#undef _WIN32_WINNT
...
bool MD5_Encrypt(const string& in, string& out)
{
    ...
    //dave 6/29/03, changed the order. the "safe" method was also really slow, so do it only if the "fast" method fails.
