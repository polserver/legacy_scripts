...
#ifdef _WIN32
void create_thread( ThreadData* td )
{
    unsigned threadid;
   ...
   // passert_always( h != 0 ); // Shinigami: doesn't help here
	CloseHandle( h );
}
#else
void create_thread( ThreadData* td )
{
   ...
   // passert_always( result == 0 ); // Shinigami: doesn't help here
}
#endif
