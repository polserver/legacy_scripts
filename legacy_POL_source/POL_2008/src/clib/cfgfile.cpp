...
ConfigProperty::ConfigProperty( const char *name, const char *value ) :
	...
	value_(value)
{
    // cerr << "creating ConfigProperty" << this << ": " << name_ << "=" << value_ << endl;
}
...
{
    // cerr << "deleting ConfigProperty" << this << ": " << name_ << "=" << value_ << endl;
}
...
/*
bool ConfigElem::remove_prop( const char *propname, char *str, int maxlen )
{
	unsigned idx;
	for( idx = 0; idx < properties.count(); idx++ )
	{
		ConfigProperty *prop = properties[idx];
		if ( stricmp( prop->name_.c_str(), propname) == 0 )
		{
			strzcpy( str, prop->value_.c_str(), maxlen );
			properties.kill( idx );
			return true;
		}
	}
