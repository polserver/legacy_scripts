...
BObjectImp* BObjectImp::unpack( const char* pstr )
{
	...
	switch( type )
	{
		case 's': return String::unpack( pstr+1 );
		case 'i': return BLong::unpack( pstr+1 );
		case 'r': return Double::unpack( pstr+1 );
		case 'u': return new UninitObject;
		//case 'a': return ObjArray::unpack( pstr+1 );
		default:
			return new BError( "Unknown object type '" + string(1,type) + "'" );
	}
}
*/
...
/*
bool BVariable::nameis( const char *testname ) const
{
	return (stricmp( name2.c_str(), testname ) == 0);
}

bool BVariable::nameis( const string& testname ) const
{
	return (stricmp( name2.c_str(), testname.c_str() ) == 0);
}
*/

/*
bool BNamedObject::nameis( const char *testname ) const
{
	return (stricmp( name2.c_str(), testname ) == 0);
}

bool BNamedObject::nameis( const string& testname ) const
{
	return (stricmp( name2.c_str(), testname.c_str() ) == 0);
}
*/
...
//const char *BObject::isA(void) const
//{
//	return objimp->isA();
//}
//bool BObject::isA(const char *s) const
//{
//	return objimp->isA( s );
//}
...
//	for_each( bobjectimp_instances.begin(), bobjectimp_instances.end(),
  //		display_bobjectimp_instance );
}
#endif
	...
	instance_(instances_++)
{
	...
	//bobjectimp_instances.in sert( this );
...
BObjectImp::~BObjectImp()
{
	...
	// bobjectimp_instances.erase( this );
	--eobject_imp_count;
}
#endif
...
BObjectRef ObjArray::OperSubscript( const BObject& rightobj )
{
	...
	if (right.isa( OTLong )) // vector
	{
		...
		{
			// ref_arr.resize( index );
			return BObjectRef( new BError( "Array index out of bounds" ) ); //TJ
		}
		else if (index <= 0)
...
BObjectImp* ObjArray::unpack( istream& is )
{
	...
	for( unsigned i = 0; i < arrsize; ++i )
	{
	   ...
	   // if ( imp->isa(OTError) )
	   // {
	   //	return new BError( "Unable to unpack array elem. Error in array elem." ); 
	   // }
...
/*
BObjectImp *ArrayElem::copy( void ) const
{
	BObjectRef *ref = objarr->refarr[index];
	if (ref) 
		return ref->obj->objimp->copy();
	else
		return new UninitObject;
}
*/
/*Object& ArrayElem::value( void ) const
{
	if (!objarr.arr[index])
		objarr.arr[index] = new UninitObject;
	return *objarr.arr[index];
}
*/
/*
void ArrayElem::assign(const Object& obj)
{
	Object *cur_obj = objarr->arr[index];
	if (cur_obj)
		delete cur_obj;
	objarr.arr[index] = obj.copy();
}
