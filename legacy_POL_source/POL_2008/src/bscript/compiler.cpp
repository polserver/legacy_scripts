...
//string Compiler::ecompile_path_em;
//string Compiler::ecompile_path_inc;
...
void Scope::addvar( const string& varname, const CompilerContext& ctx, bool warn_on_notused )
{
    ...
    // cout << "adding " << varname << " at block depth " << blockdescs_.size() << endl;
}
void Scope::addvalue()
{
...
void Expression::optimize_binary_operations()
{
    for (unsigned i = 0; i < tokens.size(); i++)
    {
        ...
        // cout << "considering optimization of (" << *left << " " << *oper << " " << *right << ")" << endl;
        Token* ntoken = NULL;
        switch( left->id )
        {
            case TOK_LONG: 
        ...
        if (ntoken)
        {
            // cout << "Replaced with " << *ntoken << endl;
            delete left;
            delete right;
            delete oper;
            tokens[ i-2 ] = ntoken;
...
void Expression::optimize_unary_operations()
{
    for (unsigned i = 0; i < tokens.size(); i++)
    {
        ...
        //cout << "considering optimization of (" << *oper << " " << *value << ")" << endl;
        Token* ntoken = NULL;
        switch( value->id )
        {
            case TOK_LONG: 
...
bool Expression::optimize_token( int i )
{
    ...
    else if (oper->id == INS_ASSIGN_CONSUME)
    {
        ...
        {
            // assigning to a global/local, then consuming.
            // we can do better, for this special case.
...
int Compiler::isFunc(Token& token, ModuleFunction **pmf)
{
	...
	{
		token.module = candidates[0].module; // WAS module,we're using relative now
        token.type = TYP_FUNC;
        token.id = TOK_FUNC;
...
/*
int Compiler::getUserArgs( Expression& ex, char **s )
{
    int i;
	int res;
	Token token;
	...
	while (1)
	{
		...
		if (tk.id == TOK_IDENT)
		{
			...
			if (tk2.id == TOK_ASSIGN)
			{
				any_named = 1;
				varname = tk.tokval();
				*s = t; // skip past the 'variable :=' part 
			}
		}
		if (varname == "")
		{
			...
			varname = userfunc->parameters[ func_params.size() ].name;
		}
		
		func_params.push_back( varname );
	...
	vector<int> param_passed( userfunc->parameters.size(), 0 );

	ex.CA.push( new Token(Mod_Basic, CTRL_MAKELOCAL, TYP_CONTROL ) );
	for( i = func_params.size()-1; (int)i >= 0; i-- )
	{
		int found;
        const string& str = func_params[i];
		found = 0;
		for( unsigned j = 0; j < userfunc->parameters.size(); j++)
		{
			UserParam& param = userfunc->parameters[j];
			if (stricmp( param.name.c_str(), str.c_str()) == 0)
			{
				found = 1;
				if (param_passed[j])
				{
					cout << "parameter passed twice\n";
					return -1;
				}
				param_passed[j] = 1;
				break;
		...
		if (!found)
		{
			cout << "parameter " << str << " is unknown\n";
			return -1;
		}
		Token *ptk = new Token( Mod_Basic, CTRL_POP_PARAM, TYP_CONTROL );
		ptk->copyStr( str.data() );
        ex.CA.push( ptk );
	}
	
	for(i = 0; i < userfunc->parameters.size(); i++)
	{
		if (param_passed[i]) continue;

		UserParam& param = userfunc->parameters[i];
		if (!param.have_default)
		{
			cout << "default param not passed\n";
			return -1;
		}
		ex.CA.push( new Token( param.dflt_value ) );
        
        //CA.put( *new Token( Mod_Basic, CTRL_POP_PARAM, TYP_CONTROL, param->name ) );
		Token *ptk = new Token( Mod_Basic, CTRL_POP_PARAM, TYP_CONTROL );
		ptk->copyStr( param.name.c_str() );
        ex.CA.push( ptk );
	}
    
	ex.CA.push( new Token( Mod_Basic, CTRL_JSR_USERFUNC, TYP_CONTROL, userfunc ) );

	return 0;
}
*/
/* Consider: when a structure member (., .+, .-. .?, etc.) is found,
   change the previous stack element's type from TOK_STRING to the member.

    FIXME: "." operator / function names
   Problem this still doesn't solve: structure elements whose name matches
   a user-defined or system-defined function. (ack!)  THis probably
   has to be handled in SmartParser::getToken...
*/
...
/*int Compiler::caseGetToken( CompilerContext& ctx, Token& token )
{
    int res = getToken( ctx, token );
    
}

int Compiler::casePeekToken( CompilerContext& ctx, Token& token )
{
    CompilerContext tctx( ctx );
    return caseGetToken( tctx, token );
}
*/
    ...
    while (!done)
    {
        ...
        while (1)
        {
            ...
            if (token.id == TOK_LONG || token.id == CTRL_LABEL || token.id == TOK_STRING)
            {
                ...
                {
                    unsigned short offset = program->tokens.next();
...
//dump(cout);
        ...
        if (patch_if_token)
        {
            ...
            tkn.offset = program->tokens.next();
...
int Compiler::handleVarDeclare( CompilerContext& ctx, unsigned save_id )
{
    ...
    do {
        ...
        */
		...
		//unsigned sympos;
        //program->symbols.append(tk_varname.tokval(), sympos);
...
int Compiler::handleConstDeclare( CompilerContext& ctx )
{
	...
	//int done = 0;
		...
		/* formats: varname followed by comma or semicolon 
              0      varname followed by "array", then by comma/semicolon
                    varname followed by ':=', then an initializer, then comma/semicolon.
        */
	    getToken(ctx, tk_varname);
		if (tk_varname.id != TOK_IDENT)
...
int Compiler::handleFor_c( CompilerContext& ctx )
{
    ...
    /* The c-style 'for' statement gets generated as follows:
       given: for( initial; predicate; iterate ) statement;
                initial;
       again:
                predicate;
...
/*
void Compiler::divine_options_in_included_file( const char *modulename )
{
    if (included.count(modulename))
        return;
    included.insert(modulename);
  
    string filename = modulename;
	filename += ".inc";
    if (!FileExists(filename.c_str()))
    {
        string try_filename = ecompile_path_inc + "/" + filename;
        if (FileExists( try_filename.c_str() ))
        {
            //cout << "Using " << try_filename << endl;
    ...
    if (getFileContents( filename.c_str(), &orig_mt ))
    {
        return;
    }
    
    CompilerContext mod_ctx( filename, orig_mt );

    inner_divine_options( mod_ctx );

    free(orig_mt);
}

void Compiler::inner_divine_options(const CompilerContext& ctx)
{
    CompilerContext tctx(ctx);
    Token tkn;
    while (getToken( tctx, tkn ) == 0)
    {
        if (tkn.id == RSV_ENDIF)
        {
            bracketed_if_ = true;
        }
        else if (tkn.id == RSV_ENDWHILE)
        {
            bracketed_while_ = true;
        }
        else if (tkn.id == RSV_ENDFOR)
        {
            bracketed_for_ = true;
        }
        else if (tkn.id == RSV_ENDFUNCTION)
        {
            bracketed_function_ = true;
        }
        else if (tkn.id == RSV_INCLUDE_FILE)
        {
            ...
            {
                divine_options_in_included_file( tk_module_name.tokval() );
            }
        }
    }
}

void Compiler::divine_options(const CompilerContext& ctx)
{
    inner_divine_options( ctx );
    included.clear();
}
*/
