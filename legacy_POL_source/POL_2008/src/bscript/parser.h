...
//  virtual int tryReservedWord(Token& tok, char *buf, char **s);
  ...
  // virtual int getToken(char **s, Token& token);
...
public:
  ...
  public:
    ...
    int IIP(Expression& expr, CompilerContext& ctx, unsigned expr_flags);
         // int comma_term_allowed, int rightparen_term_allowed
    int IP(Expression& expr, char *s);
