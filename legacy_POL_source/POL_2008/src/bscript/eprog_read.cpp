...
{
//    StoredTokenContainer ct_tokens;
//	SymbolContainer ct_symbols;
...
//    const char *bptr = symbols.array()+st.offset;

    // USED to set lval to 0 for TYP_FUNC.  Not sure if needed anymore.
    // Doesn't seem to be.
    switch( st.id )
    {
        case INS_CASEJMP:
        ...
        case TOK_USERFUNC:
            ...
            else if (token.type == TYP_FUNC)
            {
                ...
                // this used to be executed on the compiler build:
                //for( int i = 0; i < modl->functions.size(); ++i )
                //{
                //    modfunc = &modl->functions[i];
                //    if (modfunc->funcidx == token.lval)
                //        break;
                //}
