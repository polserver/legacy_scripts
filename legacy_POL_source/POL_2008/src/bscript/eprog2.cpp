...
/*
    This transform is now done in SmartParser::getToken().
			else if (token.id == TOK_ADDMEMBER ||
						 token.id == TOK_DELMEMBER ||
						 token.id == TOK_CHKMEMBER ||
						 token.id == TOK_MEMBER)
			{
				tokens.append1(
					StoredToken(token.module,
...
int EScriptProgram::write( const char *fname )
{
    ...
    hdr.globals = globalvarnames.size();
	...
	for( unsigned idx = 0; idx < program.modules.size(); idx++ )
	{
		...
		for( unsigned funcnum = 0; funcnum < module->used_functions.size(); funcnum++ )
		{
			...
			func.nargs = module->used_functions[funcnum]->nargs;
...
/*
    for( unsigned i = 0; i < blocks.size(); ++i )
    {
        ...
        if (varlast >= varfirst)
        {
            fprintf( fp, "  Local variables %d-%d: \n", varfirst, varlast );
