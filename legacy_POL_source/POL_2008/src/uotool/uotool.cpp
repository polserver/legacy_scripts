...
int print_ctable()
{
    ...
    for( int i = 0; i <= 256; i++ )
    {
        ...
        const unsigned short val = keyid[i];
...
int loschange( int argc, char **argv )
{
	...
	for( int i = 0; i <= 0xFFFF; i++ )
	{
        ...
        //bool new_lostest = ((tile.flags & USTRUCT_TILE::FLAG_NO_SHOOT) ||
        //        (tile.flags & (USTRUCT_TILE::FLAG_BLOCKING|USTRUCT_TILE::FLAG_WALL)) == (USTRUCT_TILE::FLAG_BLOCKING|USTRUCT_TILE::FLAG_WALL));
...
/*
            if (list1.size() != list2.size())
            {
                ...
                {
                    cout << "gid=" << list1[i].graphic 
                         << " z=" << int(list1[i].z) 
                         << " ht=" << int(tileheight(list1[i].graphic))
                         << endl;
                }
                cout << "list2:" << endl;
                for( i = 0; i < list2.size(); ++i )
                {
                    cout << "gid=" << list2[i].graphic 
                         << " z=" << int(list2[i].z) 
                         << " ht=" << int(tileheight(list2[i].graphic))
                         << endl;
                }
                continue;
            }
...
void print_multihull( int i, MultiDef* multi)
{
    if (multi->hull.empty())
        return;
...
void print_widedata( int i, MultiDef* multi )
{
    if (multi->hull.empty())
        return;
...
void print_multidata( int i, MultiDef* multi )
{
    if (multi->hull.empty())
        return;
...
int statics_histogram()
{
    ...
    open_uo_data_files();
    read_uo_data();
    for( int x = 0; x < 6143; x += 8 )
    {
        cout << ".";
        for( int y = 0; y < 4095; y += 8 )
        {
            USTRUCT_STATIC* p;
            int count;
...
int write_polmap( const char* filename, unsigned short xbegin, unsigned short xend)
{
    ...
    for( unsigned xs = xbegin; xs < xend; xs += 8 )
    {
        int percent = (xs-xbegin) * 100 / num;
        cout << "\r" << percent << "%";
        for( unsigned ys = 0; ys < 4096; ys += 8 )
        {
            int z;
            //USTRUCT_MAPINFO mi;
...
int wxl = 1420, wxh = 1480, wyl=1760, wyh=1780;
...
int main( int argc, char **argv )
{
    ...
    //config.uo_datafile_root = "C:/Games/uo/";
	if (argc <= 1)
		return Usage(1);
    ...
    //else if (stricmp( argv[1], "testnewstatics" ) == 0)
    //{
    //    return test_new_statics();
    //}
    ...
    {
     //   write_crypto_constants( "current.key" );
