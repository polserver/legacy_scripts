...
{
public:
    AttributeValue() : _base(0), _temp(0), _intrinsic(0) {}
...
public:
    ...
    void set_stealthsteps( unsigned newval );
    ...
    void apply_damage( double damage, Character* source = NULL );
...
private:
    ...
    friend void ClientCreateChar( Client* client, struct MSG00_CHAR_CREATE* msg);
	friend Character* CreateNewBlankCharacter();
	friend void createchar2(Account* acct, unsigned index);

private:
    //friend void handle_use_skill( Client* client, struct MSG12_EXTENDED_CMD* msg );
    //friend void skill_script_done( void *v_character );
    friend void handle_script_cursor( Character* chr, UObject* obj );
    friend void menu_selection_made( Client* client, class MenuItem* mi, struct MSG7D_MENU_SELECTION* msg );

    friend class UOExecutorModule;
    // static void skill_script_done( void *v_character );
...
//    u16 get_stamina() const { return stamina_ / 100; }
    ...
    void (*on_menu_selection)( Client *client, 
                               class MenuItem *mi, 
                               struct MSG7D_MENU_SELECTION *msg );
