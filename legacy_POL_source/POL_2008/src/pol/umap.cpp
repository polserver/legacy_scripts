...
void Map::builtin_on_use( Client* client )
{
    ...
    {
        static MSG90_MAP msg90;
        msg90.msgtype = MSGOUT_90_MAP_ID;
		...
		static MSG56_MAP_PIN msg56;
		msg56.msgtype = MSG56_MAP_PIN_ID;
		msg56.serial = serial_ext;
		msg56.type = MSG56_MAP_PIN::TYPE_REMOVE_ALL;
		msg56.pinidx = 0;
		msg56.pinx = 0;
		msg56.piny = 0;
		...
		if(!pin_points.empty())
		{
			...
			for( pin_points_itr itr = pin_points.begin(), end=pin_points.end(); itr != end; ++itr )
			{
				msg56.msgtype = MSG56_MAP_PIN_ID;
				msg56.serial = serial_ext;
				msg56.type = MSG56_MAP_PIN::TYPE_ADD;
				msg56.pinidx = 0;
...
bool Map::msgCoordsInBounds( MSG56_MAP_PIN* msg )
{
	u16 newx, newy;
...
void handle_map_pin( Client* client, MSG56_MAP_PIN* msg )
{
	...
	switch( msg->type )
	{
		case MSG56_MAP_PIN::TYPE_TOGGLE_EDIT:
			...
			msg56.type = MSG56_MAP_PIN::TYPE_TOGGLE_RESPONSE;
		...
		case MSG56_MAP_PIN::TYPE_INSERT:
			if(!(my_map->plotting))
				return;
		...
		case MSG56_MAP_PIN::TYPE_CHANGE:
			if(!(my_map->plotting))
				return;
			if(msg->pinidx >= my_map->pin_points.size()) //pinidx out of range
				return;
		...
		case MSG56_MAP_PIN::TYPE_REMOVE:
			if(!(my_map->plotting))
				return;
			if(msg->pinidx >= my_map->pin_points.size()) //pinidx out of range
				return;
		...
		case MSG56_MAP_PIN::TYPE_REMOVE_ALL:
			if(!(my_map->plotting))
				return;
...
MESSAGE_HANDLER( MSG56_MAP_PIN, handle_map_pin );
