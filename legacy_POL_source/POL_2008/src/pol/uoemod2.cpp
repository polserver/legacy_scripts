...
#define MSG3B_BUY_ITEMS_ID		  0x3B
#define MSGOUT_9E_SELL_WINDOW_ID	0x9E
#define MSG9F_SELL_ITEMS_ID		 0x9F

//
// MSG3B seems to require you use 'msglen' to figure out how many items were
// sent
//

struct MSG3B_BUY_ITEMS {
	u8  msgtype;
	u16 msglen;
	u32 vendor_serial;
	u8 status;
...
asserteql( sizeof(MSG3B_BUY_ITEMS), 15 );


struct MSG74_DESCRIBE_CONTENTS {
	struct HEADER {
		u8  msgtype;
		u16 msglen;
...
asserteql( sizeof(MSG74_DESCRIBE_CONTENTS::HEADER), 8 );
asserteql( sizeof(MSG74_DESCRIBE_CONTENTS::ELEMENT), 6 );


struct MSG9E_SELL_WINDOW {
	struct HEADER {
		u8 msgtype;
		u16 msglen;
		u32 vendor_serial;
...
asserteql( sizeof(MSG9E_SELL_WINDOW::HEADER), 9 );
asserteql( sizeof(MSG9E_SELL_WINDOW::ELEMENT), 15 );

struct MSG9F_SELL_ITEMS {
	u8 msgtype;
	u16 msglen;
	u32 vendor_serial;
	u16 num_items;
...
asserteql( sizeof(MSG9F_SELL_ITEMS), 15 );
...
bool send_vendorwindow_contents( Client* client, UContainer* for_sale, bool send_aos_tooltip )
{
		...
		MSG74_DESCRIBE_CONTENTS::HEADER* header = reinterpret_cast<MSG74_DESCRIBE_CONTENTS::HEADER*>(bfr);

		header->msgtype=MSGOUT_74_DESCRIBE_CONTENTS;
		/* Length is Below */
		...
		{
			MSG74_DESCRIBE_CONTENTS::ELEMENT* element = reinterpret_cast<MSG74_DESCRIBE_CONTENTS::ELEMENT*>(&bfr[msglen]);
			...
			element->desc_len=desc.size()+1;  //Don't forget the NULL
			element->price=ctBEu32(item->sellprice());
			msglen += addlen; // element contains first byte of desc
			++header->num_items;
...
//try this
	...
	//This looks good
	open_window.msgtype=MSGOUT_24_OPEN_GUMP;
	open_window.serial= merchant->serial_ext;
	open_window.gump=ctBEu16(0x0030);  //Serial number of the BUY gump
...
{
	MSG3B_BUY_ITEMS msg3b;
	msg3b.msgtype = MSG3B_BUY_ITEMS_ID;
	msg3b.msglen = ctBEu16(offsetof( MSG3B_BUY_ITEMS, items[0] ));
	msg3b.vendor_serial = vendor->serial_ext;
	msg3b.status = MSG3B_BUY_ITEMS::STATUS_NOTHING_BOUGHT;
	client->transmit( &msg3b, offsetof( MSG3B_BUY_ITEMS, items[0] ) );
}
...
unsigned long calculate_cost( Character* vendor, UContainer* for_sale, MSG3B_BUY_ITEMS *msg )
{
	unsigned long amt = 0;

	int nitems = (cfBEu16( msg->msglen ) - offsetof( MSG3B_BUY_ITEMS, items )) /
					 sizeof msg->items[0];
...
void buyhandler( Client* client, MSG3B_BUY_ITEMS* msg)
{
	//Close the gump

	if (msg->status == MSG3B_BUY_ITEMS::STATUS_NOTHING_BOUGHT)
		return;
...
MESSAGE_HANDLER_VARLEN(MSG3B_BUY_ITEMS, buyhandler );
...
bool send_vendorsell( Client* client, NPC* merchant, UContainer* sellfrom, bool send_aos_tooltip )
{
	...
	MSG9E_SELL_WINDOW::HEADER* header = reinterpret_cast<MSG9E_SELL_WINDOW::HEADER*>(bfr);

	header->msgtype=MSGOUT_9E_SELL_WINDOW_ID;
	/* Length is Below */
	...
	while (cont != NULL)
	{
		...
		{
			MSG9E_SELL_WINDOW::ELEMENT* element = reinterpret_cast<MSG9E_SELL_WINDOW::ELEMENT*>(&bfr[msglen]);
...
void sellhandler( Client* client, MSG9F_SELL_ITEMS* msg)
{
	UContainer* backpack = client->chr->backpack();
	if (backpack == NULL)
		return;
...
MESSAGE_HANDLER_VARLEN(MSG9F_SELL_ITEMS, sellhandler );
...
*/
	...
	MSGB0_GUMP_DIALOG::HEADER* hdr;
	MSGB0_GUMP_DIALOG::LAYOUT* playout;
	unsigned playout_idx;
	MSGB0_GUMP_DIALOG::DATA_HEADER* pdatahdr;
	unsigned pdatahdr_idx;
	MSGB0_GUMP_DIALOG::DATA* pdata;
	int msglen = 0;

	hdr = reinterpret_cast<MSGB0_GUMP_DIALOG::HEADER*>(&buffer[0]);

	hdr->msgtype = MSGB0_GUMP_DIALOG_ID;
	...
	playout = reinterpret_cast<MSGB0_GUMP_DIALOG::LAYOUT*>(&buffer[ msglen ]);
	msglen += offsetof( MSGB0_GUMP_DIALOG::LAYOUT, text );
	...
	*addpt++ = '\0';
	...
	pdatahdr_idx = msglen;
	pdatahdr = reinterpret_cast<MSGB0_GUMP_DIALOG::DATA_HEADER*>(&buffer[ msglen ]);
	int numlines = 0;
	...
	{
		pdata = reinterpret_cast<MSGB0_GUMP_DIALOG::DATA*>(&buffer[ msglen ]);
...
void gumpbutton_handler( Client* client, MSGB1_GUMP_BUTTON* msg )
{
	char* msgbuf = reinterpret_cast<char*>(msg);
	MSGB1_GUMP_BUTTON::HEADER* hdr = reinterpret_cast<MSGB1_GUMP_BUTTON::HEADER*>(msg);
	...
	// printf( "MSGB1_GUMP_BUTTON: %ld bytes\n", cfBEu16(msg->msglen) );
	// fdump( stdout, msg, cfBEu16(msg->msglen) );

	MSGB1_GUMP_BUTTON::INTS_HEADER* intshdr = reinterpret_cast<MSGB1_GUMP_BUTTON::INTS_HEADER*>(hdr+1);
	u32 ints_count = cfBEu32( intshdr->count );
	unsigned stridx = sizeof(MSGB1_GUMP_BUTTON::HEADER) +
					  sizeof(MSGB1_GUMP_BUTTON::INTS_HEADER) +
					  sizeof(MSGB1_GUMP_BUTTON::INT_ENTRY) * ints_count +
					  sizeof(MSGB1_GUMP_BUTTON::STRINGS_HEADER);
	if ( stridx > msglen )
	{
	...
	MSGB1_GUMP_BUTTON::INT_ENTRY* intentries = reinterpret_cast<MSGB1_GUMP_BUTTON::INT_ENTRY*>(intshdr+1);
	MSGB1_GUMP_BUTTON::STRINGS_HEADER* strhdr = reinterpret_cast<MSGB1_GUMP_BUTTON::STRINGS_HEADER*>(intentries+ints_count);
	u32 strings_count = cfBEu32( strhdr->count );
	// even if this is ok, it could still overflow.  Have to check each string.
	if (stridx + (sizeof(MSGB1_GUMP_BUTTON::STRING_ENTRY)-1) * strings_count > msglen+1u)
	{
	...
	else
	{
		...
		{
			MSGB1_GUMP_BUTTON::STRING_ENTRY* strentry = reinterpret_cast<MSGB1_GUMP_BUTTON::STRING_ENTRY*>(msgbuf + stridx);
			unsigned short length = cfBEu16( strentry->length );
			stridx += offsetof( MSGB1_GUMP_BUTTON::STRING_ENTRY, data ) + length * 2;
			if (stridx > msglen)
			{
...
MESSAGE_HANDLER_VARLEN(MSGB1_GUMP_BUTTON, gumpbutton_handler );
...
BObjectImp* UOExecutorModule::mf_SendTextEntryGump()
{
	...
	MSGAB_TEXTENTRY_GUMP::HEADER* phdr;
	MSGAB_TEXTENTRY_GUMP::TEXTLINE1* ptextline1;
	MSGAB_TEXTENTRY_GUMP::TEXTLINE2* ptextline2;

	phdr = reinterpret_cast<MSGAB_TEXTENTRY_GUMP::HEADER*>(&buffer[0]);
	phdr->msgtype = MSGAB_TEXTENTRY_GUMP_ID;
	...
	ptextline1 = reinterpret_cast<MSGAB_TEXTENTRY_GUMP::TEXTLINE1*>(&buffer[msglen]);
	numbytes = line1->length()+1;
	msglen += offsetof( MSGAB_TEXTENTRY_GUMP::TEXTLINE1,text ) + numbytes;
	...
	ptextline2 = reinterpret_cast<MSGAB_TEXTENTRY_GUMP::TEXTLINE2*>(&buffer[msglen]);
	numbytes = line2->length() + 1;
	if (numbytes > sizeof(ptextline2->text))
		numbytes = sizeof(ptextline2->text);
	ptextline2->cancel = static_cast<u8>(cancel);  // MSGAB_TEXTENTRY_GUMP::TEXTLINE2::CANCEL_DISABLE;
	ptextline2->style = static_cast<u8>(style); // MSGAB_TEXTENTRY_GUMP::TEXTLINE2::STYLE_NORMAL;
	ptextline2->mask = ctBEu32(maximum);
	ptextline2->numbytes = ctBEu16( numbytes );
	memcpy( ptextline2->text, line2->data(), numbytes );
	msglen += offsetof( MSGAB_TEXTENTRY_GUMP::TEXTLINE2,text) + numbytes;
...
void handle_textentry( Client* client, MSGAC_TEXTENTRY_RESULT* msg )
{
	...
	if (msg->retcode == MSGAC_TEXTENTRY_RESULT::RETCODE_OKAY)
	{
...
MESSAGE_HANDLER_VARLEN( MSGAC_TEXTENTRY_RESULT, handle_textentry );
...
BObjectImp* UOExecutorModule::mf_CreateAccount()
{
	...
	if (getStringParam( 0, acctname ) &&
		...
		getParam( 2, enabled ))
	{
		// Scott 10/10/2007 Added control char check to block exploits from www/aux/etc
		// 10/11 Optimized it a touch. Ewww, ugly code.
		if (acctname->alnumlen() < acctname->length() ) 
		{
			return new BError( "Attempted to create account with non-alphanumeric characters." );	
		}
		const char *tstr = password->data();
		for ( unsigned int i = 0; i < password->length(); ++i )
		{
			if (!isgraph(tstr[i]) || iscntrl(tstr[i])
	        || tstr[i] == '}'
		    || tstr[i] == '{')
			{
...
void handle_resurrect_menu( Client *client, MSG2C_RESURRECT_MENU *msg )
{
	if (msg->choice)
	{
...
MESSAGE_HANDLER( MSG2C_RESURRECT_MENU, handle_resurrect_menu );
...
BObjectImp* UOExecutorModule::mf_SendInstaResDialog()
{
	...
	MSG2C_RESURRECT_MENU msg;
	msg.msgtype = MSG2C_RESURRECT_MENU_ID;
...
void handle_selcolor( Client* client, MSG95_DYE_VAT* msg )
{
	if (client->chr != NULL &&
		client->gd != NULL &&
...
MESSAGE_HANDLER( MSG95_DYE_VAT, handle_selcolor );
...
BObjectImp* UOExecutorModule::mf_SendOpenBook()
{
	...
	msg.msgtype = MSG93_OPEN_BOOK_ID;
	...
	if (writable)
	{
		//PolTimer timer;
		unsigned msglen = 0;
		MSG66_BOOK_CONTENTS_HDR* phdr = reinterpret_cast<MSG66_BOOK_CONTENTS_HDR*>(buffer);
		phdr->msgtype = MSG66_READ_BOOK_PAGE_ID;
		phdr->msglen = 0;
		...
		{
			MSG66_BOOK_CONTENTS_PAGE* ppage = reinterpret_cast<MSG66_BOOK_CONTENTS_PAGE*>(&buffer[msglen]);
			msglen += sizeof(*ppage);
...
void read_book_page_handler( Client* client, MSG66_READ_BOOK_PAGE* msg )
{
	...
	else
	{
		...
		size_t bytesleft = msglen - offsetof( MSG66_READ_BOOK_PAGE, text );
		for( int i = 1; i <= 8; ++i )
		{
			string line;
			while (bytesleft)
...
MESSAGE_HANDLER_VARLEN( MSG66_READ_BOOK_PAGE, read_book_page_handler );
...
void open_book_handler( Client* client, MSG93_OPEN_BOOK* msg )
{
...
MESSAGE_HANDLER( MSG93_OPEN_BOOK, open_book_handler );
...
BObjectImp* UOExecutorModule::mf_SendHousingTool()
{
	...
	msg.subcmd = ctBEu16(MSGBF_GENERIC_COMMAND::TYPE_ACTIVATE_CUSTOM_HOUSE_TOOL);
...
BObjectImp* UOExecutorModule::mf_SendCharacterRaceChanger(/* Character */)
{
	...
	if (getCharacterParam( exec, 0, chr ))
	{
		MSGBF_GENERIC_COMMAND msg;
		msg.msgtype = MSGBF_GENERIC_COMMAND_ID;
		msg.subcmd = ctBEu16(MSGBF_GENERIC_COMMAND::TYPE_CHARACTER_RACE_CHANGER);
		msg.characterracechanger.call.gender = chr->gender;
		msg.characterracechanger.call.race = chr->race+1;
...
void character_race_changer_handler( Client* client, MSGBF_GENERIC_COMMAND* msg )
{
	...
	client->chr->setcolor(cfBEu16(msg->characterracechanger.result.BodyHue));
