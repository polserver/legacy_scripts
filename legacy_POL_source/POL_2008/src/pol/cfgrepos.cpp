...
StoredConfigElem::StoredConfigElem( ConfigElem& elem )
{
	...
	{
		BObjectImp* newimp = bobject_from_string( propval );
...
void StoredConfigElem::addprop( const std::string& propname, BObjectImp* imp )
{
	propimps_.insert( PropImpList::value_type( propname.c_str(), ref_ptr<class BObjectImp>(imp) ) );
}
...
BObjectImp* StoredConfigElem::listprops( ) const
{
...
void StoredConfigFile::load_tus_scp( const std::string& filename )
{
	...
	while (getline(ifs,strbuf))
	{
		if (strbuf[0] == '[')
		{
			...
			elemref->addprop( "_key", bobject_from_string( strbuf, 16 ) );
		}
		...
		BObjectImp* newimp = bobject_from_string( propvalue, 16 );
...
ConfigFileRef FindConfigFile( const string& filename, const string& allpkgbase )
{
	...
	try {
		if ( !allpkgbase.empty() )
		{
			...
			for( Packages::iterator itr = packages.begin(); itr != packages.end(); ++itr )
			{
				...
				if ( FileExists(pkgfilename.c_str()) )
				{
					ConfigFile cf( pkgfilename.c_str() );
					scfg->load( cf );
					any = true;
				}
			}
		...
		else
		{
			...
			{
				if (mlog.is_open())
					mlog << "Config File " << filename << " does not exist." << endl;
				return ConfigFileRef(0);
			}
	...
	catch( exception& ex )
	{
		...
		if (mlog.is_open())
			mlog << "An exception was encountered while reading " << filename << ": " << ex.what() << endl;
		return ConfigFileRef(0);
	}
}
