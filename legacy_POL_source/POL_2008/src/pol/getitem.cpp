...
//                     line 19x for a dirty fix for instadecay on items dropped in current
...
void get_item( Client *client, MSG07_GET_ITEM *msg )
{
	...
	if (pol_distance( client->chr, item ) > 2)
	{
	...
	int orig_x = item->x, orig_y = item->y, orig_z = item->z;
...
MESSAGE_HANDLER( MSG07_GET_ITEM, get_item );
