...
//needed?
//bool CCryptBlowfish::m_bTablesReady = false;
...
void CCryptBlowfish::Decrypt(void *pvIn, void *pvOut, int len)
{
	...
	if(m_type == typeLogin)
	{
		...
		for(tempPos = 0; tempPos < len; tempPos++)
		{
			...
			m_loginKey[1] = (((((table[1] >> 1) | (table[0] << 31)) ^ m_masterKey[0]) >> 1) | (table[0] << 31)) ^ m_masterKey[0];
			m_loginKey[0] = ((table[0] >> 1) | (table[1] << 31)) ^ m_masterKey[1];
		}
	}
	else
	{
...
void CCryptBlowfish::InitTables()
{
	int tempKey;
...
int CCryptBlowfish::Pack(void *pvIn, void *pvOut, int len)
{
	unsigned char *pIn = (unsigned char *)pvIn;
	unsigned char *pOut = (unsigned char *)pvOut;
...
void CCryptBlowfish::RawDecrypt(unsigned int *pValues, int table)
{
