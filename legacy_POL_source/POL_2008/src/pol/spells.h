...
extern int spell_scroll_objtype_limits[7][2];

class InternalSpellHandler
{
public:
    InternalSpellHandler( const char *name, void (*f)(Character* caster) );
    InternalSpellHandler( const char *name, TargetCursor* tcursor );
};
...
inline unsigned long USpell::delay() const
{
    return params_.delay;
}
inline unsigned USpell::difficulty() const
{
