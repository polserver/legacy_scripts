...
{
	//dave 6/5/3 messed with this, so we always store hashes and sometimes cleartext
	if( elem.has_prop("Password") )
	{
		...
		if(!MD5_Encrypt(temppass,password_only_hash_))	//MD5
			elem.throw_error("Failed to encrypt password for " + name_);
		accounts_txt_dirty = true;
	}
	else if( elem.has_prop("PasswordOnlyHash") && elem.has_prop("PasswordHash") )
	{
		passwordhash_ = elem.remove_string( "PasswordHash" );
		password_only_hash_ = elem.remove_string( "PasswordOnlyHash" );
	}
	else if( elem.has_prop("PasswordHash") )
	{
		passwordhash_ = elem.remove_string( "PasswordHash" );
		//password_only_hash_ remains empty since we don't know the cleartext pass
	}
	else
...
void Account::writeto( std::ostream& os )
{
	...
	if(!password_only_hash_.empty())
		os << "\tPasswordOnlyHash\t" << password_only_hash_ << std::endl;  //MD5
...
void Account::writeto( ConfigElem& elem ) const
{
	...
	if(!password_only_hash_.empty())
        elem.add_prop( "PasswordOnlyHash", password_only_hash_.c_str() );
...
const string Account::password_only_hash() const
{
    return password_only_hash_;
}
