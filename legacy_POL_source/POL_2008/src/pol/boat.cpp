...
/*
            string defstr;
            if (elem.remove_prop( boat_components[i].name, &defstr ))
                objdefs[i] = ObjDef( defstr );
*/
        } 
        catch ( exception& ex )
        {
...
/* // used to remove offline characters here
        else if (IsCharacter(obj->serial))
        {
            Character* chr = static_cast<Character*>(obj);
...
void UBoat::adjust_traveller_z(signed int delta_z)
{
    for( Travellers::iterator itr = travellers_.begin(), end = travellers_.end();
         itr != end;
         ++itr )
...
void UBoat::rescan_components()
{
    ...
    if (plank)
        plank->setboat( this );
    //for( int i = 0; i < N_BOAT_COMPONENTS; ++i )
    //{
    //    components_[i] = NULL;
    //}

    //const BoatDef& bdef = boatdef();
}
...
void UBoat::readProperties( ConfigElem& elem )
{
    ...
    while (elem.remove_prop( "Traveller", &tmp_serial ))
    {
        ...
        {
            Character* chr = system_find_mobile( tmp_serial, SYSFIND_SEARCH_OFFLINE_MOBILES );
