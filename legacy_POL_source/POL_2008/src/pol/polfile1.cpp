...
{
    //unsigned long pflags = tile_uoflags( pstat->graphic );
    char pheight = tileheight( pstat->graphic );

//    if (! (pflags & USTRUCT_TILE::FLAG_WALKBLOCK) )
//    {
//        return true;
//    }
    ...
    for( unsigned i = 0; i < vec.size(); ++i )
    {
        ...
        // unsigned long flags = tile_flags( prec.graphic );
...
int write_pol_static_files( const string& realm )
{
    ...
    for( unsigned y = 0; y < descriptor.height; y += STATICBLOCK_CHUNK )
    {
        ...
        for( unsigned x = 0; x < descriptor.width; x += STATICBLOCK_CHUNK )
        {
            STATIC_INDEX idx;
            idx.index = index;
            ...
            int num;
            // unsigned long offset = ftell( fdat );
