...
template<>
int TmplExecutorModule<HttpExecutorModule>::function_table_size =
...
void send_binary( Socket& sck, const string& page, const string& filename, const string& content_type )
{
	...
	if ( ifs.is_open() )
	{
		...
		while ( cur_read < fsize )
		{
			ifs.readsome(bfr, sizeof bfr);
			cur_read += ifs.gcount();
			sck.send(bfr, sizeof bfr);
		}		
		// -------------
		http_writeline( sck, "" );
	}
...
{
	pthread_t th;
	passert_always( pthread_create( &th, 
									&http_attr, 
									http_conn_thread_stub, 
									reinterpret_cast<void*>(client_socket) ) == 0);
}
