...
/*Character* CreateNewBlankCharacter(u16 objtype)
{
	Character* chr = new Character(objtype);
...
void ClientCreateChar( Client* client, MSG00_CHAR_CREATE* msg)
{
	if ( client->acct == NULL )
	{
	...
	else if ( config.min_cmdlevel_to_login > 0 )
	{
		client->disconnect = 1;
		return;
	}
	...
	else if ( client->acct->active_character != NULL ) // this account has a currently active character.
	{
...
//	Character* chr = CreateNewBlankCharacter( ((msg->Sex & FLAG_GENDER) == FLAG_GENDER) ?
	...
	// Scott 10/11/2007 Added sanity checks for control characters.
	string tmpstr(msg->Name, sizeof msg->Name);
	const char *tstr = tmpstr.c_str();
	//cout << "Testing '" << tstr << "' for invalid characters. Length='"<<strlen(tstr)<<"'\n";
...
/*
	if (msg->Strength + msg->Intelligence + msg->Dexterity != ssopt.total_stats_at_creation)
	{
		cerr << "Create Character: Stats sum to " 
			 << msg->Strength + msg->Intelligence + msg->Dexterity
			 << ".  (Expected " << ssopt.total_stats_at_creation << ")" << endl;
		client->disconnect = 1;
		return;
	}
*/
	...
	if (msg->SkillNumber1 > uoclient_general.maxskills || //dave changed 3/15/03, support configurable max skillid
	...
	//Create Starting_gold gold in your pack (This is probably not right)
	// MuadDib: Changed to use a ssopt entry to allow admins to customize
	//		  starting gold amounts. :(
...
//		tmpitem->newbie(true);
	...
	//Right now clothes and a dagger
	//Create shoes(Layer 3)
	...
	//Create dagger(Layer 1 - right hand)
	...
	{
		// Gals get shirt and skirt
		pantstype = 0x1516;
		shirttype = 0x1517;
	}
	else
	{
		//Guys get shirt and pants
		pantstype = 0x152e;
		shirttype = 0x1517;
	}

	// printf( "Pants: 0x%x, Shirt: 0x%x\n", msg->pantscolor, msg->shirtcolor );
	//Create Pants (Layer 4)
	...
	// parameters are (character, skillid-array)
...
MESSAGE_HANDLER(MSG00_CHAR_CREATE, ClientCreateChar);
