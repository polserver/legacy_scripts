// History
//  2006/05/16 Shinigami: updated doubleclick() to support Elfs
...
void send_paperdoll( Client *client, Character *chr )
{
	...
	msg.msgtype = MSGOUT_88_PAPERDOLL;
	msg.serial = chr->serial_ext;
	
	// FIXME if this is my paperdoll, tell me all about me.
	// if this is someone else's, tell me less.

	string name = (chr->title_prefix.empty() ? "": chr->title_prefix + " ") + chr->name() + 
...
void doubleclick( Client *client, MSG06_DOUBLE_CLICK *msg )
{
...
MESSAGE_HANDLER( MSG06_DOUBLE_CLICK, doubleclick );
