...
void read_pol_config( bool initial_load )
{
    ...
    if (initial_load)
    {
        ...
        config.use_new_statics_file = elem.remove_bool( "UseNewStaticsFile", false );
        config.use_pol_map_file = elem.remove_bool( "UsePolMapFile", false );
	...
	config.minidump_type = elem.remove_string("MiniDumpType","small");
	config.retain_cleartext_passwords = elem.remove_bool( "RetainCleartextPasswords", true); //dave added 6/5/3
#ifdef _WIN32
