...
WeatherRegion::WeatherRegion( ConfigElem& elem ) :
    Region( elem ),
    weathertype( elem.remove_ushort( "WeatherType", 255 ) ), //dave changed 6/30/03, 255 is no weather, not 0
    severity( elem.remove_ushort( "WeatherSeverity", 0 ) ),
    aux( elem.remove_ushort( "WeatherAux", 0 ) ),
...
bool WeatherDef::assign_zones_to_region(
								 ...
								 Realm* realm)
{
    if (xwest  >= realm->width()) xwest  = realm->width()-1;
    if (xeast  >= realm->width()) xeast  = realm->width()-1;
    if (ynorth >= realm->height()) ynorth = realm->height()-1;
    if (ysouth >= realm->height()) ysouth = realm->height()-1;
