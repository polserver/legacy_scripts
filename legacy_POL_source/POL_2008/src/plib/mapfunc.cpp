...
u32 polflags_from_tileflags( unsigned tile, u32 uoflags, bool use_no_shoot, bool LOS_through_windows )
{
    ...
    if ((uoflags & USTRUCT_TILE::FLAG_LIQUID) &&
        (uoflags & USTRUCT_TILE::FLAG_FLOOR))
    {
        mapflags |= FLAG::MOVESEA;
    }
//    else if ((uoflags & USTRUCT_TILE::FLAG_LIQUID) &&
...
/*
else if ((uoflags & USTRUCT_TILE::FLAG_FLOOR) &&
        (uoflags & USTRUCT_TILE::FLAG_BLOCKING))
    {
        // example: cedar tree, 0xcd6
    }
    else if ((uoflags & USTRUCT_TILE::FLAG__WALK) &&
        (uoflags & USTRUCT_TILE::FLAG_PLATFORM) &&
        (~uoflags & USTRUCT_TILE::FLAG_BLOCKING))
    {
        // example: stairs at 5338, 46
        mapflags |= FLAG::MOVELAND;
    }
    else if ((uoflags & USTRUCT_TILE::FLAG__WALK) &&
        (uoflags & USTRUCT_TILE::FLAG_FLOOR) &&
...
// TODO: add to test suite
    ...
    else if ((uoflags & USTRUCT_TILE::FLAG_WALL) &&
        (~uoflags & USTRUCT_TILE::FLAG_BLOCKING) &&
        (uoflags & USTRUCT_TILE::FLAG__WALK))
    {
        // example: 0x245 dungeon arch (is not a support)
    }
    else if ((uoflags & USTRUCT_TILE::FLAG__WALK) &&
        (~uoflags & USTRUCT_TILE::FLAG_BLOCKING))
    {
        // example: a fallen log (0xcf3) at 3116, 73
        mapflags |= FLAG::MOVELAND;
    }
    else if ((uoflags & USTRUCT_TILE::FLAG_NO_SHOOT) &&
        (uoflags & USTRUCT_TILE::FLAG_BLOCKING))
    {
        // example: 5159,15
    }
...
*/
	...
	{
		//dave commented out lines with FLAG__WALK 9/6/03, it really has nothing to do with movement or LOS
		if ((uoflags & USTRUCT_TILE::FLAG_WALL) &&
			(~uoflags & USTRUCT_TILE::FLAG_BLOCKING) &&
			//(uoflags & USTRUCT_TILE::FLAG__WALK) &&
    ...
    if (flags_set( uoflags, USTRUCT_TILE::FLAG_FLOOR|USTRUCT_TILE::FLAG_LIQUID|USTRUCT_TILE::FLAG_BLOCKING))
    {
        // not blocking
    }
    else if ((uoflags & USTRUCT_TILE::FLAG_BLOCKING))// &&
    ...
    else if (flags_clear( uoflags, USTRUCT_TILE::FLAG_WALL) &&
             flags_clear( uoflags, USTRUCT_TILE::FLAG_BLOCKING))
    {
	...
	// Shinigami: transformed to a warning
   // passert_always( ! ((mapflags & FLAG::BLOCKING) &&
   //                (mapflags & (FLAG::MOVELAND|FLAG::MOVESEA))) );
