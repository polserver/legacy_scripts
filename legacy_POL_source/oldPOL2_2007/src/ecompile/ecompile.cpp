...
bool quiet = false;
//bool warnings = false;
//bool generate_lstfile = false;
//bool generate_dbgfile = false;
//bool generate_txt_dbgfile = false;
bool opt_generate_wordlist = false;
//bool compile_asp_pages = false;
...
void usage(void)
{
...
#ifdef WIN32
    cerr << "       -Pdir        path to EM and INC files" << endl;
#endif
 ...
 // cerr << "       -z           include non-intrusive debug information in .ecl file" << endl;
	cerr << endl;
	cerr << " NOTE:" << endl;
...
bool compile_file(const char *path)
{
        ...
        //cout << "Tokens: " << Token::instances() << endl;
        ...
        //cout << "Tokens: " << Token::instances() << endl;
        //cout << "UserFunctions: " << UserFunction::instances() << endl;
        //Token::show_instances();

    return true;
    //UserFunction::show_instances();
}
...
//                case 'z':
//                    ecompile_include_debug_sections = true;
//                    break;
                ...
                case 'P':
                    {
                        ...
                        putenv( EmPathEnv.c_str() );
                        putenv( IncPathEnv.c_str() );
                    }
                    break;
#endif
...
{
//    PushDir pd( basedir.c_str() );
//    if (!pd.ok()) // not a directory, bail out
//        return;
//    basedir = curdir();
    ...
    for( DirList dl( basedir.c_str() ); !dl.at_end(); dl.next() )
    {
        ...
        {
            ++summary.CompiledScripts; // well, we did try
            ++summary.ScriptsWithCompileErrors;
            if (!keep_building)
                throw;
			s_errors++;
...
void AutoCompile()
{
    ...
    // compile scripts and enabled packages
...
int xmain(int argc, char *argv[])
{
    ...
    if (!quiet)
    {
        ...
        cerr << "Copyright (C) 1994-2007 Eric N. Swanson" << endl;
        cerr << endl;
    }
