...
/*class VariableNotDeclaredException
{
    const char *varname
    VariableNotDeclaredException( const char *varname )
        enum {
    }
...
class Executor
{
  ...
  public:
    ...
    // BVariable* findVariable( const char* varName);
    BObject* makeObj( const Token& token);
	int makeGlobal( const Token& token );
//	int makeLocal( const Token& token );
...
//	int declareArray( const Token& token );

    BObjectRef addmember( BObject& left, const BObject& right );
    void addmember2( BObject& left, const BObject& right );

    // execmodules: modules associated with the
    //  current program.  References modules owned by availmodules.
