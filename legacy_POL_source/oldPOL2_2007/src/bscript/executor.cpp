...
bool Executor::AttachFunctionalityModules()
{
	for( unsigned midx = 0; midx < prog_->modules.size(); midx++ )
	{
		FunctionalityModule* fm = prog_->modules[ midx ];
            ...
            // if no function in the module is actually called, don't go
            // searching for it.
        ...
        if (!fm->have_indexes)
        {
                // Possible optimization: store these function indexes in the
...
int Executor::getParams(unsigned howMany)
{
    if (howMany)
	{
		...
		for(int i = howMany - 1; i >= 0; --i) 
		{
			...
			if (objref.get() == (BObject *) NULL) 
            {
				printerror("Too few parameters on stack! (serious)");
				return -1;
			}
			parm[i] = objref;
*/
...
/*
    for(int i=0; i < howMany; i++) 
	{
        if (parm[i].get() == NULL) 
		{
           printerror("Parameter is NULL! (serious)");
           passert(0);
           return -1;
        } 
		else 
...
/*BVariable *Executor::findVariable(const char *varName)
{
    int count, i;
         // If there are any local variables, look there first
    if (Locals2) 
	{
         count = Locals2->size();
         for(i = count-1; i >= 0; --i) 
		 {
             BVariable* var = &(*Locals2)[i];
             if (var->nameis(varName)) return var;
         }
    }
         // No local variable 'varName' in scope, try a global 

    count = Globals2.size();
    for(i=0;i<count;i++) 
	{
        BVariable* var = &Globals2[i];
        if (var->nameis( varName))
            return var;
    }
        // Not found.  This is unacceptable. 
    cerr << "Variable '" << varName << "' has not been declared." << endl;

    seterror( true );
        // Not found.  create it, local if possible 
    VarArray2 *va;
    if (Locals2) va = Locals2;
    else va = &Globals2;

    //BVariable *newVar = new BVariable;
    //newVar->set( new BObject( new UninitObject ) );
    //newVar->name = varName;
    
    va->push_back( BVariable() );
	va->back().name2 = varName;
	va->back().set( new BObject( new UninitObject ) );
	return &va->back();
    //return newVar;
}
*/
...
void Executor::ins_declareArray( const Instruction& ins )
{
	...
	{
		cerr << "variable is already initialized.." << endl; // FIXME weak error message
		seterror( true );
		return;
	}
...
void Executor::getArg( const Token& token )
{
    ...
    else
    {
        ...
        /* we assume pass by value, for the time being. */
    
        /*
	    BVariable *newvar = new BVariable;
        newvar->name2 = token.tokval();
        newvar->set( new BObject( objref->impptr()->copy() ) );
	    */
        Locals2->push_back( BObjectRef() );
	    Locals2->back().set( new BObject( objref->impptr()->copy() ) );
    }
...
BObjectRef Executor::addmember( BObject& left, const BObject& right )
{
	...
	{	// TODO: ??
		return BObjectRef(left.clone());// new BObject( left.impref().copy() );
	}
...
/*
    if (left.isa( BObjectImp::OTArray ))
    {
		ObjArray& objarr = static_cast<ObjArray&>(left.impref());

        if (objarr.addMember( varname.data(), new UninitObject ))
        {
            //InformDebuggerNewMember( (Variable *) pleft, av );
            return objarr.ref_arr.back();
        }
        else
        {
            return BObjectRef(left.clone()); // new BObject(left.objimp->copy());
        }
...
BObject* ArrayIterator::step()
{
    ...
    if (++m_Index > m_pArray->ref_arr.size())
        return NULL; // do nothing, we're done
...
void Executor::ins_set_member( const Instruction& ins )
{
    ...
    if (right.count() == 1 && rightimpref.count() == 1)
    {
      ...
      // left = BObject( rightimpref.solo_assign() );
    }
    else
    {
...
void Executor::ins_lessequal( const Instruction& ins )
{
    ...
    //int _true = (left->isLessEqual(right.impref()));
	leftref.set( new BObject( new BLong( _true ) ) );
}
...
{
    // the subscripts are on the value stack in right-to-left order, followed by
    // the array itself
...
void Executor::ins_dictionary_addmember( const Instruction& ins )
{
    ...
    // we don't have to check the type because the compiler is in charge.
...
void Executor::innerExec(const Instruction& ins)
{
    ...
    switch(ins.token.id) 
    {
        ...
        case TOK_IDENT:
			ValueStack.push( BObjectRef( new BObject( new BError( "Please recompile this script" ) ) ) );
			//ValueStack.push( BObjectRef( findVariable(token.tokval())->get() ) );
            return;
     	...
     	case INS_ASSIGN_CONSUME:    // ins_assign_consume
            {
                ...
                {
                    left.setimp( &rightimpref );
                    //left.impref().specialAssign( left, *rightimpref.solo_assign() );
                }
                else
                {
                    left.setimp( rightimpref.copy() );
                    //left->specialAssign( left, *rightimpref.copy() );
                }
                ValueStack.pop();
			  ...
			  case TOK_ASSIGN:
                  {
                      ...
                      else
                      {
                          ...
                          //left = BObject(rightimpref.copy());
					                ...
					                // if (!quiet) cout << "now LEFT isa " << left.value().isA() << endl;
                      return;
                  }
                break;
			  case TOK_ADD:         // ins_add
...
//    case TOK_MEMBER:            return &Executor::ins_member;
