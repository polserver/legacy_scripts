...
class CompilerScope
{
    ...
    vector<CompilerVariable> variables;
};
*/
...
struct BlockDesc
{
    ...
    // eb_type break_continue_ok;          // block, while..
...
private:
    //static string ecompile_path_em;
    //static string ecompile_path_inc;
...
//    typedef vector<UserFunction> UserFuncs;
//  UserFuncs v_userfuncs;
    ...
    Variables globals_;
    //typedef vector<string> GlobalVars;
    ...
    */
    ...
    void rollback( EScriptProgram& prog, const EScriptProgramCheckpoint& checkpoint );
             // stack<LocalScope> scopes;
    /*
    bool inGlobalScope() const
    {
        return (scopes.size() <= 1 &&
                !currentscope->inblock());
    }
    */
...
private:
    /*
        bool bracketed_if_;
        bool bracketed_while_;
        bool bracketed_for_;
        bool bracketed_function_;
    */
