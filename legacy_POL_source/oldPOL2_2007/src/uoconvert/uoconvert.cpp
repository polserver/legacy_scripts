...
int xmain( int argc, char* argv[] )
{
    ...
    else if (command == "statics")
    {
        ...
        uo_map_width = descriptor.width;
        uo_map_height = descriptor.height;
...
void ProcessSolidBlock( unsigned x_base, unsigned y_base, MapWriter& mapwriter )
{
    ...
    for( unsigned x_add = 0; x_add < x_add_max; ++x_add )
    {
        for( unsigned y_add = 0; y_add < y_add_max; ++y_add )
        {
            ...
            cell.z = base.z; //assume now map has height=1. a static was already added if it was >0
            ...
            if (!shapes.empty())
            {
                ...
                for( int j = 0; j < count; ++j )
                {
                    ...
                    char z, height, flags;
                    z = shapes[j].z;
                    //passert_always( shape.flags & (FLAG::BLOCKING|FLAG::MOVELAND|FLAG::MOVESEA|FLAG::BLOCKSIGHT) );
                    //passert_always( shape.flags & FLAG::BLOCKSIGHT );
				   // Shinigami: commented out - not transformed to a warning like cfg_show_roof_and_platform_warning
                    //passert_always( ! ((shape.flags & FLAG::BLOCKING) &&
                    //                  (shape.flags & (FLAG::MOVELAND|FLAG::MOVESEA))) );
                    height = shape.height;
...
void create_multis_cfg( FILE* multi_idx, FILE* multi_mul, FILE* multis_cfg )
{
    ...
    for( int i = 0; fread( &idxrec, sizeof idxrec, 1, multi_idx ) == 1; ++i )
    {
        ...
        {
//            cout << "MultiID " << i << " in verdata.mul" << endl;
...
//            cout << "MultiID " << i << " in multi.mul" << endl;
