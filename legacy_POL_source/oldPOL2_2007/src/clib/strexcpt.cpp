...
void segv_handler(int signal)
{
    if (signal == SIGSEGV)
	{
        Log( "Caught SIGSEGV (Segfault).  Please mail the following to pol-core-test:\n" );
        cerr << "Caught SIGSEGV (Segfault).  Please mail the following to pol-core-test:" << endl;
	}
    else
	{
        Log( "Caught SIGUSR2 (On-demand backtrace).  Please mail the following with explanation and last lines from log files to pol-core-test:\n" );
        cerr << "Caught SIGUSR2 (On-demand backtrace).  Please mail the following with explanation and last lines from log files to pol-core-test:" << endl;
	}
	Log( "=== CUT ===\n" );
   ...
   // backtrace_symbols_fd( bt, n, STDERR_FILENO );
    ...
    if (signal == SIGSEGV)
    {
        ...
        // this doesn't seem to work - just outputs 'Killed'
