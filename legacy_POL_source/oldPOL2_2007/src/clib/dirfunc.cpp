...
//static Pathname fullpath;
//static Pathname workspace;
...
/*
    sprintf(workspace,"%s\\%s.%d",directory, filename,node);
    _fullpath(fullpath, workspace, sizeof fullpath);

    assert(strcmp(fullpath, fullpath2)==0);
*/
...
/*
    sprintf(workspace,
           (filename && filename[0]) ? "%s\\%s" : "%s",
           (directory && directory[0]) ? directory : ".", filename);
    _fullpath(fullpath, workspace, sizeof fullpath);


    assert(strcmp(fullpath2, fullpath)==0);
*/
    return fullpath2;
}
