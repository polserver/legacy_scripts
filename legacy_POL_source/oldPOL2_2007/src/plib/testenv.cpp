...
void add_multi( unsigned short objtype, int x, int y, int z, long flags )
{
    ...
    BObject obj( UMulti::scripted_create( find_itemdesc(objtype), x, y, z, realm, flags ) );
}
...
NPC* add_npc( const char* npctype, int x, int y, int z )
{
    ConfigFile cfile;
    ConfigElem elem;
    if (!FindNpcTemplate( npctype, cfile, elem ))
    {
...
void create_test_environment()
{
    ...
    add_multi( 0x6040, 1487, 1768, -5, CRMULTI_FACING_NORTH );
    ...
    add_multi( 0x6044, 1495, 1817, -5, CRMULTI_FACING_WEST );
