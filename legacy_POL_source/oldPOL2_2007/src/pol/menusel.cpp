...
void handle_menu_selection( Client *client, MSG7D_MENU_SELECTION *msg )
{
    ...
    // Client chose from the right menu.  He can't again.
	client->chr->menu = NULL;

	if (msg->choice == 0)	// client cancelled menu
	{
		// printf( "Client cancelled menu\n" );
        client->chr->cancel_menu();
		return;
	}
	...
	if (mi->submenu_id)
	{
		// printf( "Client selected submenu %d\n", mi->submenu_id );
		client->chr->menu = find_menu( mi->submenu_id );
		send_menu( client,  client->chr->menu);
	}
	else
	{
		// printf( "Client chose a leaf menu selection (objtype 0x%x)\n", mi->objtype_ );
...
MESSAGE_HANDLER( MSG7D_MENU_SELECTION, handle_menu_selection );
