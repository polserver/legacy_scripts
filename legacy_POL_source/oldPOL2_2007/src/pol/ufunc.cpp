...
/*	
	// FIXME to use transmit_to_inrange
    for( Clients::iterator itr = clients.begin(), end = clients.end(); itr != end; ++itr )
    {
        Client *client = *itr;
        if (!client->ready)
						 ...
						 Realm* realm)
{
    ...
    for( Clients::iterator itr = clients.begin(), end = clients.end(); itr != end; ++itr )
    {
        ...
        if (!client->ready)
            continue;
        // fixme need a visual range check here.
...
{
// FIXME : does not work.......
	if (client->getversion().substr(0,2) != "1.")
    {
		MSGOUT_BC_SEASON_INFO msg;
        msg.msgtype = MSGOUT_BC_SEASON_INFO_ID;
		//msg.season = MSGOUT_BC_SEASON_INFO::SEASON_SPRING;
		//msg.change = MSGOUT_BC_SEASON_INFO::NO_CHANGE;
