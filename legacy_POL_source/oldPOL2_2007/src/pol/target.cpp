...
void handle_target_cursor( Client *client, MSG6C_TARGET_CURSOR *msg )
{
...
MESSAGE_HANDLER( MSG6C_TARGET_CURSOR, handle_target_cursor );
/*
...
/*
struct {
	void (*func)(Client *client, MSG6C_TARGET_CURSOR *msg);
...
void TargetCursor::handle_target_cursor( Character* chr, MSG6C_TARGET_CURSOR* msg )
{
...
FullMsgTargetCursor::FullMsgTargetCursor( void (*func)(Character*, MSG6C_TARGET_CURSOR*) ) :
    TargetCursor( false /* don't inform on cancel */ ),
    func(func)
{
}

void FullMsgTargetCursor::on_target_cursor( Character* chr, MSG6C_TARGET_CURSOR* msg )
{
    (*func)(chr, msg);
}
...
void LosCheckedTargetCursor::on_target_cursor( Character* chr, MSG6C_TARGET_CURSOR* msgin )
{
    if (msgin == NULL)
    {
        (*func)(chr,NULL);
...
void NoLosCheckedTargetCursor::on_target_cursor( Character* chr, MSG6C_TARGET_CURSOR* msgin )
{
    if (msgin == NULL)
    {
        (*func)(chr,NULL);
...
LosCheckedCoordCursor::LosCheckedCoordCursor( 
                            void (*func)(Character*,MSG6C_TARGET_CURSOR*),
                            bool inform_on_cancel ) :
    TargetCursor( inform_on_cancel ),
    func_(func)
{
...
bool LosCheckedCoordCursor::send_coord_cursor( Client* client )
{
	if (!client->chr->target_cursor_busy())
    {
		static MSG6C_TARGET_CURSOR msg;
		msg.msgtype = MSG6C_TARGET_CURSOR_ID;
		msg.unk1 = MSG6C_TARGET_CURSOR::UNK1_01;
		msg.target_cursor_serial = ctBEu32( cursorid_ );	
		msg.cursor_type = MSG6C_TARGET_CURSOR::CURSOR_TYPE_NEUTRAL;
		client->transmit( &msg, sizeof msg );
		client->chr->tcursor2 = this;
		return true;
	}
...
void LosCheckedCoordCursor::on_target_cursor( Character* chr, MSG6C_TARGET_CURSOR* msg )
{
    (*func_)(chr,msg);
}
...
MultiPlacementCursor::MultiPlacementCursor( void (*func)(Character*, MSG6C_TARGET_CURSOR*) ) :
    TargetCursor( true ),
    func_(func)
{
}
...
{
    MSG99_MULTI_PLACEMENT msg;
	memset(&msg,0,sizeof msg);
    msg.msgtype = MSGOUT_99_MULTI_PLACEMENT_ID;
    msg.unk1_01 = 0x01;
...
void MultiPlacementCursor::on_target_cursor( Character* chr, MSG6C_TARGET_CURSOR* msg )
{
    (*func_)(chr,msg);
}
...
void NoLosCharacterCursor::on_target_cursor( Character* targetter, MSG6C_TARGET_CURSOR* msgin )
{
    if (msgin == NULL)
        return;
    u32 selected_serial = cfBEu32( msgin->selected_serial );
...
void NoLosUObjectCursor::on_target_cursor( Character* targetter, MSG6C_TARGET_CURSOR* msgin )
{
    if (msgin == NULL)
    {
        (*func)(targetter,NULL);
