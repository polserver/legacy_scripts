...
bool statics_height( unsigned short x, unsigned short y, int oldz, int* newz )
{
    ...
    fseek( sidxfile, 
           x_block * 512 * sizeof idxrec + 
             y_block * sizeof idxrec, 
           SEEK_SET );
    ...
    if (idxrec.length != 0xFFffFFffLu)
    {
        ...
        for( idxrec.length /= 7; idxrec.length > 0; --idxrec.length )
        {
            ...
            {
                signed char ztemp;
                cout << "static: objtype=0x" << ios::hex << srec.graphic << ios::dec 
                     << ", z=" << int(srec.z) 
                     << ", ht=" << int(tileheight(srec.graphic)) << endl;
                ztemp = srec.z + tileheight(srec.graphic);
                
                if ( (ztemp <= oldz+7) &&   // not too high to step onto
                     (ztemp > z))           // but above the highest yet seen
                {
                    cout << "Setting Z to " << int(ztemp) << endl;
                    z=ztemp;
                }
            }
...
/*
    if (newz == -127)
    {
        result = groundheight( x, y, &newz );
        if (result && lowest_blocking_z <= newz+10 && highest_blocking_z > newz)
            result = false;
    }
*/
