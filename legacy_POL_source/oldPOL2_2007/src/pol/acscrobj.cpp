...
#include "clib/MD5.h"
///$TITLE=Account Scripting Object
...
class AccountMemberRef : public B
{
public:
	AccountMemberRef( T& in_acct_member );	
	
	void* operator new( size_t len ) { return ::operator new(len); }
	void operator delete( void* vp, size_t len ) { ::operator delete(vp,len); }
	void operator delete( void* vp ) { ::operator delete(vp); }

	virtual bool useSpecialAssign() const;
...
///	 All methods except GetProp and GetCharacter write the data/accounts.txt file on success.
...
BObjectImp* AccountObjImp::call_method_id( const int id, Executor& ex )
{
	...
	case MTH_SETPASSWORD:
		if (ex.numParams() == 1)
		{
			...
			if (ex.getStringParam( 0, pwstr ))
			{
				...
				MD5_Encrypt(pwstr->value(),temp);
				obj_->password_only_hash_ = temp; //MD5
				break;
			}
			else
			{
	...
	case MTH_SETNAME:
		...
		if ( (ex.numParams() == 1) && config.retain_cleartext_passwords )
		{
			...
			if (ex.getStringParam( 0, nmstr ))
			{
				...
				std::string temp;
				//assing the new name, and recalc name+pass hash (pass only hash is unchanged)
	...
	case MTH_GETCHARACTER:
		...
		if (!ex.getParam( 0, index, 1, MAX_CHARS ))
			return NULL;
	...
	case MTH_SET_UO_EXPANSION:
		...
		{
			if (expansion_str->value().empty() || (expansion_str->value()=="ML") ||
			   ...
			   (expansion_str->value()=="LBR") || (expansion_str->value()=="T2A"))
			{
				...
				return new BError( "Invalid Parameter Value. Supported Values: \"\", T2A, LBR, AOS, SE, ML" );
		}
		else
			return new BError("Invalid Parameter Type");
		break;
	...
	case MTH_SPLIT:
		if( ex.numParams() == 2 )
		{
			...
			if (ex.getStringParam( 0, acctname ) && 
				ex.getParam( 1, index, 1, MAX_CHARS ))
			{
	...
	{
		long index;
		if ( !ex.getParam(0, index, 0, MAX_CHARS) )
...
BObjectRef AccountObjImp::get_member_id( const int id ) //id test
{
	switch(id)
	{
		...
		case MBR_BANNED: 
			...
			break;
		case MBR_PASSWORDHASH: 
			return BObjectRef( new String( obj_->password_only_hash() ) ); //dave added 6/5/3
			break;
		case MBR_USERNAMEPASSWORDHASH: 
			return BObjectRef( new String( obj_->passwordhash() ) ); //dave added 6/5/3
			break;
		case MBR_UO_EXPANSION: 
			return BObjectRef( new String( obj_->uo_expansion() ) ); //dave added 6/5/3
			break;
