...
{
    string filename;
    // map1 uses map0 + 'dif' files
    if (uo_mapid != 1)
        filename = "map" + tostring(uo_mapid) + ".mul";
    else
        filename = "map0.mul";
	mapfile     = open_uo_file( filename );

    if (uo_mapid != 1)
        filename = "staidx" + tostring(uo_mapid) + ".mul";
    else
        filename = "staidx0.mul";
    sidxfile    = open_uo_file( filename );

    if (uo_mapid != 1)
        filename = "statics" + tostring(uo_mapid) + ".mul";
    else
        filename = "statics0.mul";
    statfile    = open_uo_file( filename );
