...
///	 /pol/data/ds/fname.txt
///	 /pol/data/ds/{pkgname}/fname.txt
///
		...
		getParam( 1, flags ))
	{
		try
		{
			...
			else
			{
				...
				descriptor = ":" + spec_pkg->name() + ":" + spec_filename;
				d_ds = "data/ds/";
				directory = "data/ds/" + spec_pkg->name() + "/";
			}
...
{
	if (!FileExists( "data/datastore.txt" ))
		return;
	
	ConfigFile cf( "data/datastore.txt", "DataFile" );
	ConfigElem elem;
