...
void Spellbook::double_click( Client* client )
{
	...
	{
		send_sysmessage( client, "This item requires at least the Age of Empires Expanion." );
		return;
	}
	else if( !(client->UOExpansionFlag & SE) && 
		     (spell_school == 4 || spell_school == 5))
	{
		 send_sysmessage( client, "This item requires at least the Samurai Empire Expanion." );
		return;
	}
	else if( !(client->UOExpansionFlag & ML) && 
		     spell_school == 6)
	{
		 send_sysmessage( client, "This item requires at least the Mondain's Legacy Expanion." );
		return;
	}
	else
	{
		...
		msg.subcmd = ctBEu16(MSGBF_GENERIC_COMMAND::TYPE_NEW_SPELLBOOK);
