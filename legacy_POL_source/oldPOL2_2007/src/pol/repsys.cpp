...
void load_repsys_cfg_namecoloring( ConfigElem& elem )
{
...
void load_repsys_cfg_general( ConfigElem& elem )
{
...
void load_repsys_cfg_hooks(ConfigElem& elem)
{
	string temp;
...
{
	ConfigFile cf("config/repsys.cfg");
	
	ConfigSection namecoloring(cf, "NameColoring");
...
void RepSystem::repsys_task( Character* amy )
{
	...
	Character::CharacterSet defensive_hostiles;
	for( Character::CharacterSet::iterator h_itr = amy->opponent_of.begin(); h_itr != amy->opponent_of.end(); ++h_itr )
	{
		Character* hostile_bob = *h_itr;
...
// Changed to update!
				...
				Log( "NOTE: repsys crash avoidance...\n" );
...
// Changed to update!
	...
	for( Character::CharacterSet::iterator h_itr = defensive_hostiles.begin(); h_itr != defensive_hostiles.end(); ++h_itr )
	{
		Character* bob = *h_itr;
...
void RepSystem::schedule_repsys_task( Character* chr, polclock_t runat )
{
	...
	if (chr->repsys_task_ == NULL)
	{
		new OneShotTaskInst<Character*>( &chr->repsys_task_,
										 runat,
...
void RepSystem::on_pc_attacks_pc( Character* amy_attacker, Character* bob_defender )
{
...
///	 
/// 
void RepSystem::on_pc_damages_pc( Character* amy, Character* bob )
{
	if (amy == bob)
		return;
...
///	
void RepSystem::on_pc_helps_pc( Character* amy, Character* bob )
{
...
void RepSystem::de_escalate( Character* amy, Character* bob )
{
...
unsigned char RepSystem::hilite_color_idx( const Character* amy, const Character* bob )
{
	
	if ( repsys_cfg.Hooks.HighLightColor )
	{
		Character* t_amy = const_cast<Character*>(amy);
		Character* t_bob = const_cast<Character*>(bob);
 ...
 *
 */
unsigned short RepSystem::name_color( const Character* amy, const Character* bob )
{
...
bool Character::is_aggressor_to( const Character* chr ) const
{
	...
	return ( aggressor_to_.find( CharacterRef(in_chr) ) != aggressor_to_.end());
}
...
//	
//
void Character::restart_aggressor_timer( Character* amy, polclock_t until )
{
	MobileCont::iterator itr = aggressor_to_.find( CharacterRef(amy) );
	if (itr == aggressor_to_.end())
	{
		aggressor_to_[ CharacterRef(amy) ] = until;
	}
	else
	{
		if (until > (*itr).second)
...
{
	Character* in_chr = const_cast<Character*>(chr);
	return ( lawfully_damaged_.find( CharacterRef(in_chr) ) != lawfully_damaged_.end() );
}

void Character::restart_lawfully_damaged_timer( Character* amy, polclock_t when )
{
	MobileCont::iterator itr = lawfully_damaged_.find( CharacterRef(amy) );
	if (itr == lawfully_damaged_.end())
	{
		lawfully_damaged_[ CharacterRef(amy) ] = when;
	}
	else
	{
		if (when > (*itr).second)
...
{
	return (polclock() <= criminal_until_) || murderer_;
}

void Character::restart_criminal_timer( polclock_t until )
{
	if (until > criminal_until_)
	{
		if (criminal_until_ < polclock())
		{
...
void Character::repsys_on_attack( Character* defender )
{
	...
	{
		RepSystem::on_pc_attacks_pc( this, defender );
	}
	else
	{
		...
		if (npc->master())
		{
			RepSystem::on_pc_attacks_pc( this, npc->master() );
		}
		else if ((npc->alignment() == NpcTemplate::GOOD) && (!npc->is_criminal()))
		{
			make_criminal();
		}
	}
...
void Character::repsys_on_damage( Character* defender )
{
	...
	if (defender->isa( CLASS_CHARACTER ))
	{
		RepSystem::on_pc_damages_pc( this, defender );
	}
	else
	{
		NPC* npc = static_cast<NPC*>(defender);
		if (npc->master())
		{
			RepSystem::on_pc_damages_pc( this, npc->master() );
		}
		else if ((npc->alignment() == NpcTemplate::GOOD) && (!npc->is_criminal()))
		{
			make_criminal();
...
void Character::repsys_on_help( Character* helped )
{
	...
	{
		RepSystem::on_pc_helps_pc( this, helped );
	}
	else
	{
		...
		if (npc->master())
		{
			RepSystem::on_pc_helps_pc( this, npc->master() );
		}
		else if (npc->alignment() == NpcTemplate::EVIL)
		{
			make_criminal();
		}
	}
...
{
	return RepSystem::hilite_color_idx( seen_by, this );
}
unsigned short Character::name_color( const Character* seen_by ) const
{
	return RepSystem::name_color( seen_by, this );
}
...
unsigned char NPC::hilite_color_idx( const Character* seen_by ) const
{
	...
	if ( master() )
	{
		return RepSystem::hilite_color_idx( seen_by, master() );
	}
	else
	{
...
void Character::clear_reportable( u32 serial, polclock_t when )
{
	set_dirty();
	reportable_t rt;
	rt.serial = serial;
...
void Character::clear_my_aggressors()
{
	...
	{ 
		u32 serial = (*itr);
		Character* aggressor = system_find_mobile( serial );
		if (aggressor != NULL)
		{
...
void Character::clear_my_lawful_damagers()
{
	...
	{ 
		u32 serial = (*itr);
		Character* damager = system_find_mobile( serial );
		if (damager != NULL)
		{
