...
{
	if (FileExists( "data/impworld.wsc" ))
	{
		cout << "Found data/impworld.wsc - importing" << endl;
		if (1)
		{
			ConfigFile cf( "data/impworld.wsc" );
...
Item* read_item( ConfigElem& elem )
{
	...
	{
		if (system_find_item( serial, SYSFIND_SEARCH_OFFLINE_MOBILES | SYSFIND_SEARCH_STORAGE_AREAS ))
		{
...
void read_global_item( ConfigElem& elem, int sysfind_flags )
{
	...
	else
	{
		if (IsCharacter( container_serial )) // it's equipped on a character
		{
			Character* chr = system_find_mobile( container_serial, SYSFIND_SEARCH_OFFLINE_MOBILES );
			if (chr != NULL)
			{
		...
		if (cont_item == NULL)
		{
			cont_item = system_find_item( container_serial, sysfind_flags | SYSFIND_SEARCH_OFFLINE_MOBILES );
...
void read_pol_dat()
{
	slurp( "data/pol.txt", "GLOBALPROPERTIES SYSTEM" );

	if (polvar.DataWrittenBy == 0)
	{
		cerr << "CoreVersion not found in data/pol.txt" << endl << endl;
		cerr << "data.pol.txt must contain a section similar to: " << endl;
		cerr << "System" << endl
			 << "{" << endl
			 << "	CoreVersion 93" << endl
			 << "}" << endl
...
{
	slurp( "data/objects.txt", "CHARACTER NPC ITEM GLOBALPROPERTIES" );
}

void read_pcs_dat()
{
	slurp( "data/pcs.txt", "CHARACTER ITEM", SYSFIND_SKIP_WORLD );
}

void read_pcequip_dat()
{
	slurp( "data/pcequip.txt", "ITEM", SYSFIND_SKIP_WORLD );
}

void read_npcs_dat()
{
	slurp( "data/npcs.txt", "NPC ITEM", SYSFIND_SKIP_WORLD );
}

void read_npcequip_dat()
{
	slurp( "data/npcequip.txt", "ITEM", SYSFIND_SKIP_WORLD );
}
	
void read_items_dat()
{
	slurp( "data/items.txt", "ITEM" );
}

void read_multis_dat()
{
	slurp( "data/multis.txt", "MULTI" );
//	if (FileExists( "data/multis.txt" ))
//	{
//		cout << "data/multis.txt:";
//		ConfigFile cf( "data/multis.txt", "MULTI" );
//		ConfigElem elem;
//		while( cf.read( elem ))
//		{
...
{
	if (FileExists( "data/storage.txt" ))
	{
		cout << "  data/storage.txt:";
		ConfigFile cf2( "data/storage.txt" );
		storage.read( cf2 );
	}
}
...
{
	if (FileExists( "data/import.txt" ))
	{
		if (1)
		{
			ConfigFile cf( "data/import.txt", "ITEM" );
			ConfigElem elem;
			while (cf.read( elem ))
			{
				import( elem );
...
{
	string datname = "data/" + basename + ".dat";
	string txtname = "data/" + basename + ".txt";
...
int read_data()
{
	...
	if (FileExists( "data/objects.ndt" ))
	{
	...
	if (FileExists( "data/storage.ndt" ))
	{
		cerr << "Error!" << endl
			 << "'data/storage.ndt exists.  This probably means the system" 
			 << endl
			 << "exited while writing its state.  To avoid loss of data," 
			 << endl
			 << "forcing human intervention."
	...
	ofs_datastore()
{
	...
	ofs_pol.open(		"data/pol.ndt",		 ios::out );
	ofs_objects.open(	"data/objects.ndt",	 ios::out );
	ofs_pcs.open(		"data/pcs.ndt",		 ios::out );
	ofs_pcequip.open(	"data/pcequip.ndt",	 ios::out );
	ofs_npcs.open(	   "data/npcs.ndt",		ios::out );
	ofs_npcequip.open(   "data/npcequip.ndt",	ios::out );
	ofs_items.open(	  "data/items.ndt",	   ios::out );
	ofs_multis.open(	 "data/multis.ndt",	  ios::out );
	ofs_storage.open(	"data/storage.ndt",	 ios::out );
	ofs_resource.open(   "data/resource.ndt",	ios::out );
	ofs_guilds.open(	 "data/guilds.ndt",	  ios::out );
	ofs_datastore.open(  "data/datastore.ndt",   ios::out );
...
{
	string bakfile = "data/" + basename + ".bak";
	string datfile = "data/" + basename + ".txt";
	string ndtfile = "data/" + basename + ".ndt";
