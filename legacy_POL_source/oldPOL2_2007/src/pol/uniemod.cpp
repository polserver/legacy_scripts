...
{
    static MSGC2_UNICODE_PROMPT msg;
	memset(&msg, 0, sizeof msg);
    msg.msgtype = MSGC2_UNICODE_PROMPT_ID;
    msg.msglen = ctBEu16( MSGC2_UNICODE_PROMPT::SERVER_MSGLEN );
    msg.serial = serial;
	msg.msg_id = serial; // Server-"decided" message ID. o_O
	transmit( client, &msg, MSGC2_UNICODE_PROMPT::SERVER_MSGLEN );
}

void handle_unicode_prompt( Client* client, MSGC2_UNICODE_PROMPT* msg )
{
    UnicodeExecutorModule* uniemod = client->gd->prompt_uniemod;
    if (uniemod == NULL)
        return; // log it?

	int textlen = ((cfBEu16(msg->msglen) - offsetof(MSGC2_UNICODE_PROMPT,wtext))
...
MESSAGE_HANDLER_VARLEN( MSGC2_UNICODE_PROMPT, handle_unicode_prompt );
...
BObjectImp* UnicodeExecutorModule::mf_PrintTextAboveUC()
{
        ...
        getParam( 4, color ))
    {
		...
		return new BLong( say_above( obj, gwtext, strupper(lang->value()).c_str(), font, color ) );
    }
    else
    {
        return new BError( "A parameter was invalid" );
