...
BObjectImp* NPCExecutorModule::CanMove()
{
    if (exec.fparams.size() == 1)
    {
        ...
        else if (param0->isa( BObjectImp::OTLong ))
        {
            ...
            UFACING facing = static_cast<UFACING>(blong->value() & MSG02_FACING_MASK);
            return new BLong(npc.could_move(facing)?1:0);
        }
        else
        {
...
bool NPCExecutorModule::_internal_move(UFACING facing, int run)
{
	...
	int dir;
	dir = facing;
	if (run) dir |= 0x80; // FIXME HARDCODE

	if (npc.could_move(facing))
	{
		if (npc.move(dir)) // this could still fail, if paralyzed or frozen
		{
			npc.tellmove(); // FIXME return 0 on failure?
			//move was successful
			success = true;
		}
		else
		{
			//npc could move, but move failed.
			success = false;
		}
	}
	else
...
BObjectImp* NPCExecutorModule::move_self( UFACING facing, bool run, bool adjust_ok )
{
    ...
    {
		if (npc.anchor_allows_move(facing) && npc.move(dir))
		{
			npc.tellmove(); // FIXME return 0 on failure
			success = true;
		}
...
BObjectImp* NPCExecutorModule::mf_Wander()
{
    ...
    switch( random_int( 8 ) )
    {
        case 0: case 1: case 2: case 3: case 4: case 5:
            ...
            break;
        case 6:
            newfacing = (static_cast<int>(npc.facing)-1) & MSG02_FACING_MASK;
            adjust_ok = false;
            break;
        case 7:
            newfacing = (static_cast<int>(npc.facing)+1) & MSG02_FACING_MASK;
...
BObjectImp* NPCExecutorModule::say()
{
	...
	//talkmsg.textdef.type = TEXTTYPE_NORMAL;
	talkmsg.textdef.color = ctBEu16( npc.speech_color_ );
	talkmsg.textdef.font = ctBEu16( npc.speech_font_ );
	strzcpy( talkmsg.speaker_name, npc.name().c_str(), sizeof talkmsg.speaker_name );
	strzcpy( talkmsg.text, text, sizeof talkmsg.text );
	unsigned short msglen = offsetof( MSG1C_TEXT, text ) + strlen( talkmsg.text ) + 1;
	talkmsg.msglen = ctBEu16( msglen );

	if ( texttype_str == "default" )
		talkmsg.textdef.type = TEXTTYPE_NORMAL;
	else if ( texttype_str == "whisper" )
		talkmsg.textdef.type = TEXTTYPE_WHISPER;
	else if ( texttype_str == "yell" )
		talkmsg.textdef.type = TEXTTYPE_YELL;
	else
	...
	for( unsigned cli = 0; cli < clients.size(); cli++ )
	{
		...
		bool rangeok;
		if ( talkmsg.textdef.type == TEXTTYPE_WHISPER )
			rangeok = in_whisper_range(&npc, client->chr);
		else if ( talkmsg.textdef.type == TEXTTYPE_YELL )
	...
	if ( doevent >= 1 )
		for_nearby_npcs(npc_spoke, &npc, text, strlen(text), talkmsg.textdef.type);
...
BObjectImp* NPCExecutorModule::SayUC()
{
        ...
        getParam( 3, doevent))
    {
		...
		{
			msg.textdef.type = TEXTTYPE_YELL;
		}
		else if ( texttype_str == "whisper" )
		{
			msg.textdef.type = TEXTTYPE_WHISPER;
		}
		else
		{
			msg.textdef.type = TEXTTYPE_NORMAL;
		}
		msg.textdef.color = ctBEu16( npc.speech_color_ );
		msg.textdef.font = ctBEu16( npc.speech_font_ );
		...
		for( Clients::iterator itr = clients.begin(), end = clients.end(); itr != end; ++itr )
	    {
			...
			bool rangeok;
			if ( msg.textdef.type == TEXTTYPE_WHISPER )
				rangeok = in_whisper_range(&npc, client->chr);
			else if ( msg.textdef.type == TEXTTYPE_YELL )
				rangeok = in_yell_range(&npc, client->chr);
			else
				rangeok = in_say_range(&npc, client->chr);
...
void NPC::readNpcProperties( ConfigElem& elem )
{
    ...
    string tmp;
    if (elem.remove_prop( "AR", &tmp ))
    {
        Dice dice;
        string errmsg;
        if (!dice.load( tmp.c_str(), &errmsg ))
        {
            cerr << "Error reading AR for NPC: " << errmsg << endl;
    ...
    {
		Character* chr = system_find_mobile(  master_serial, SYSFIND_SEARCH_OFFLINE_MOBILES );
        if (chr != NULL)
            master_.set( chr );
    }
...
void NPC::inform_moved( Character* moved )
{
    ...
    if (ex != NULL)
    {
        ...
        else if ( ex->eventmask & (EVID_ENTEREDAREA|EVID_LEFTAREA) )
        {
            passert( this != NULL );
            passert( moved != NULL );
            ...
            else if ( !are_inrange && were_inrange )
            {
				if ( (!ssopt.event_visibility_core_checks) || is_visible_to_me( moved ) )
