// History
//  2006/05/16 Shinigami: createchar() updated to support Elfs
...
void createchar( Client* client, MSG00_CHAR_CREATE* msg)
{
	Character* chr;
	...
	if (config.min_cmdlevel_to_login > 0)
	{
		client->disconnect = 1;
		return;
	}

	if (msg->CharNumber >= MAX_CHARS ||
	...
	chr = new Character( ((msg->Sex & FLAG_GENDER) == FLAG_GENDER) ?
	...
	// FIXME An attempt at bounds checking.
	// TODO validate characters in the name
	string tmpstr(msg->Name, sizeof msg->Name);
	chr->name_ = tmpstr.c_str();
...
MESSAGE_HANDLER(MSG00_CHAR_CREATE, createchar );
