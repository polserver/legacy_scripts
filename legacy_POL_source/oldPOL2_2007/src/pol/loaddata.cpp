...
void load_incremental_indexes() // indices is such a stupid word
{
	for(;;)
	{
		...
		string filename = "data/incr-index-" + decint(next_incremental_counter) + ".txt";
...
void read_incremental_saves()
{
	...
	{
		string filename = "data/incr-data-" + decint(i) + ".txt";
...
void insert_deferred_items()
{
	...
	for( DeferList::iterator itr = deferred_insertions.begin(); itr != deferred_insertions.end(); ++itr )
	{
		...
		{
			Item* cont_item = system_find_item( container_serial, SYSFIND_SEARCH_OFFLINE_MOBILES );
			Item* item = static_cast<Item*>(obj);
			if (cont_item != NULL)
			{
