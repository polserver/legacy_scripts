...
{
	//dave 6/5/3, added timing info
	unsigned long naccounts = 0;
    static int num_until_dot = 1000;
	wallclock_t start = wallclock();
    cout << "  " << "data/accounts.txt" << ":";
    stat( "data/accounts.txt", &accounts_txt_stat );

	{
		ConfigFile cf( "data/accounts.txt", "Account" );
		ConfigElem elem;
...
{
    RemoveFile( "data/accounts.bak" );
    RemoveFile( "data/accounts.ndt" );
    
    {
        ofstream ofs( "data/accounts.ndt", std::ios::trunc|ios::out );
    ...
    rename( "data/accounts.txt", "data/accounts.bak" );
    rename( "data/accounts.ndt", "data/accounts.txt" );

	struct stat newst;
    stat( "data/accounts.txt", &newst );
