...
/*static bool read_newer_tile( int tilenum, USTRUCT_TILE* tile )
{
    long int pos, block;
    ...
    {
        pos = 4 + (sizeof *tile) * (tilenum & 0x1F);
...
//        return static_cast<unsigned char>(ceil(static_cast<double>(height)/2.0));
        return (height/2);
    else
        return height;
}
...
{
    for( unsigned graphic = 0; graphic < N_TILEDATA; ++graphic )
    {
...
{
    for( unsigned objtype = 0; objtype < N_TILEDATA; ++objtype )
    {
