...
//
// History:
// - 15. Jun. 2000 : Quick, dirty hack to make crypt-class work with the Ignition tool
...
static unsigned char b_box[80] =
{
	0x45, 0x6e, 0x63, 0x72, 0x79, 0x70, 0x74, 0x69, 0x6f, 0x6e, 0x2d, 0x6d, 0x6f, 0x64, 0x75, 0x6c, 
	0x65, 0x20, 0x63, 0x6F, 0x64, 0x65, 0x64, 0x20, 0x62, 0x79, 0x20, 0x42, 0x65, 0x6f, 0x73, 0x69, 
	0x6c, 0x2e, 0x20, 0x54, 0x68, 0x69, 0x73, 0x20, 0x63, 0x6f, 0x64, 0x65, 0x20, 0x6d, 0x61, 0x79, 
...
static unsigned long s_box[1024] = 
{
...
void CCrypt::Decrypt(void *pvIn, void *pvOut, int len)
{
/*
	...
	if(m_type == typeLogin)
	{
		...
		{
			pOut[tempPos] = pIn[tempPos] ^ (unsigned char)m_loginKey[0];

			unsigned int table[2];
			table[0] = m_loginKey[0];
			table[1] = m_loginKey[1];

			m_loginKey[1] = (((((table[1] >> 1) | (table[0] << 31)) ^ CRYPT_LOGINKEY_1) >> 1) | (table[0] << 31)) ^ CRYPT_LOGINKEY_1;
			m_loginKey[0] = ((table[0] >> 1) | (table[1] << 31)) ^ CRYPT_LOGINKEY_2;
		}
	}
	else
	{
		if(m_gameStreamPos + len > CRYPT_GAMETABLE_TRIGGER)
		{
			int lenOld = CRYPT_GAMETABLE_TRIGGER - m_gameStreamPos;

			Decrypt(pIn, pOut, lenOld);
		...
		for(tempPos = 0; tempPos < len; tempPos++)
		{
			...
			unsigned char c = *pIn++;
			*pOut++ = m_gameSeed[m_gameBlockPos] ^ c;
...
void CCrypt::Init(void *pvSeed, int type)
{
...
int CCrypt::Pack(void *pvIn, void *pvOut, int len)
{
	unsigned char *pIn = (unsigned char *)pvIn;
	unsigned char *pOut = (unsigned char *)pvOut;
...
{
	unsigned int left = pValues[0];
	unsigned int right = pValues[1];
